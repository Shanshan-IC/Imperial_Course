...
in_range(Min, Max, N):- X is (Min+1), X =< Max, in_range(X, Max, N).

%Question b
empty_cell(N):- cell(N), \+ prisoner(_, _, N, _, _, _).

%Question c
all_female_cell(N):- cell(N), prisoner(_, _, N, _, _, _), forall(prisoner(_, Name, N, _, _, _), female_name(Name)).

%Question d
female_prisoners(N1):- findall(N, (prisoner(_, Name, N, _, _, _), female_name(Name)), L), length(L, N1). 

%Question e
cell_occupancy(Cell, N):- cell(Cell), findall(Name, prisoner(_, Name, Cell, _, _, _), L), length(L, N).

%Question f
size_list(L):- setof(S, Cell^S1^(cell_occupancy(Cell, S1),S is -S1), L).
fullest_cell(N):- cell(N), size_list([H|L]), Num is -H, cell_occupancy(N, Num).
%model answer
%fullest_cell(N):- cell_occupancy(N, Num), \+(cell_occupancy(_, M), M > N).
%Question g
worst_psychopath(S,F,Crime,T):- psychopath(S, F), prisoner(S, F, _, _, L, _), forall((psychopath(S1, F1), prisoner(S1, F1, _, _, L1, _), S1 \= S), L1 > L).

%Question h
criminals(C, N):- crimes(L), member(C, L), findall(Name, (prisoner(_, Name, _, C, _, _)), CL), length(CL, N).
